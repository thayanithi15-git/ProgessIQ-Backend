...
const bigquery = require("../config/db");
 ...
 */
function frontendReturnUrl(params: Record<string, any>) {
 ...
 * still gets JSON.
 */
function respondWithError(req: any, res: any, { status, reason, error, details }: { status: number; reason: string; error: string; details?: string }) {
  if (req.method === "POST") {
 ...
 * ============================================================
 *
 * Example: <prefix_state>:<account_identifier>
 */
 ...
 * ============================================================
 *
 * Validates against AMAZON_STATE environment variable
 */
 ...
 *   &state=STATE_VALUE
 */
const handleAmazonCallback = async (req: any, res: any) => {
  try {
    /**
     * --------------------------------------------------------
