...
/**
 * ============================================================
 * Normalize the OAuth redirect URI
 * ============================================================
 *
 * Amazon compares this against the redirect_uri used at
 * authorize time byte for byte, and answers invalid_grant on
 * any difference. A doubled slash from joining a base URL to a
 * path is the usual culprit, so it is collapsed here.
 *
 * The frontend normalizes identically before sending the user
 * to Amazon, so both halves of the flow agree.
 */
function normalizeRedirectUri(value: string | undefined): string {
  ...
  return raw
    // Collapse repeated slashes in the path, leaving "https://" intact.
    .replace(/([^:])\/{2,}/g, "$1/")
    .replace(/\/+$/, "");
}
 ...
 * BigQuery columns:
 *
 * refresh_token      returned by Amazon
 * client_id          NULL for now, reserved for the LWA
 *                    client id once it is captured
 * account_identifier the SP-API application ID that was
 *                    authorized
 * account_type       seller / vendor
 * selling_partner_id the Amazon account that authorized
 * created_at         written by this query
 *
 * Example:
 *
 * Seller:
 *   account_identifier = amzn1.sp.solution.cfa5c707...
 *   account_type       = seller
 *
 * Vendor:
 *   account_identifier = amzn1.sp.solution.dc6747e9...
 *   account_type       = vendor
 *
 * A refresh token belongs to one (application, selling
 * partner) pair, so that pair is the merge key. Re-authorizing
 * updates the existing row; a new pair inserts one.
 */
async function saveAmazonToken({
  ...
  /**
   * Rows written before this layout carry a NULL
   * selling_partner_id. `=` is NULL-unsafe in BigQuery, so a
   ...
   * deterministic.
   *
   * client_id is written as NULL on purpose. The column is
   * reserved for the LWA client id; until that is captured,
   * writing it on both branches keeps re-authorization
   * self-healing for rows the previous layout filled with the
   * application ID.
   */
  const query = `
  ...
  await bigquery.query({
    ...
    params: {
      ...
      account_identifier: String(applicationId),
...
export const handleAmazonCallback = async (req: Request, res: Response) => {
  try {
     ...
     *
     * client_id:
     *   NULL for now, reserved for the LWA client id
     *
     * account_identifier:
     *   the SP-API application ID that was authorized
     *
     * account_type:
     *   seller / vendor
     *
    ...
    if (req.method === "POST") {
      return res.status(200).json({
        ...
        data: {
          client_id: null,
          account_identifier: selectedApplicationId,
