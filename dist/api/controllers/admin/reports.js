...
exports.getPlatforms = exports.getMentorsReport = exports.getYears = exports.getDepartments = exports.getReportStats = exports.deleteReport = exports.downloadReport = exports.getReportHistory = exports.previewReport = exports.generateReport = void 0;
...
const buildQuery = (category, filters) => {
    const query = {};
    // Date filters
    ...
    if (filters.minPoints !== undefined || filters.maxPoints !== undefined) {
        ...
        if (filters.minPoints !== undefined)
            query.totalPoints.$gte = filters.minPoints;
        if (filters.maxPoints !== undefined)
            query.totalPoints.$lte = filters.maxPoints;
    }
    // Designation filter
...
const getReportData = async (category, filters) => {
    ...
    switch (category) {
        case 'students':
            data = await Student_1.default.find(query).sort(sort).limit(filters.limit || 1000).populate('userId', 'name email');
            totalRecords = await Student_1.default.countDocuments(query);
            break;
        case 'mentors':
            data = await Mentor_1.default.find(query).sort(sort).limit(filters.limit || 1000).populate('userId', 'name email');
            totalRecords = await Mentor_1.default.countDocuments(query);
            break;
        case 'projects':
            data = await Project_1.default.find(query).sort(sort).limit(filters.limit || 1000)
                .populate('studentId', 'name department year')
                .populate('mentorId', 'name');
            totalRecords = await Project_1.default.countDocuments(query);
            break;
        case 'internships':
            data = await Internship_1.default.find(query).sort(sort).limit(filters.limit || 1000)
                .populate('studentId', 'name department year');
            totalRecords = await Internship_1.default.countDocuments(query);
            break;
        case 'certifications':
            data = await Certification_1.default.find(query).sort(sort).limit(filters.limit || 1000)
                .populate('studentId', 'name department year');
            totalRecords = await Certification_1.default.countDocuments(query);
            break;
        case 'performance':
            // Get students with their points
            const students = await Student_1.default.find(query).sort(sort).limit(filters.limit || 1000)
                .populate('userId', 'name email');
            data = await Promise.all(students.map(async (student) => {
                const points = await Point_1.default.find({ studentId: student._id });
        ...
        case 'comprehensive':
            ...
            const [comprehensiveStudents, comprehensiveMentors, comprehensiveProjects, comprehensiveInternships, comprehensiveCertifications] = await Promise.all([
                Student_1.default.find(query).limit(500).populate('userId', 'name email'),
                Mentor_1.default.find(query).limit(500).populate('userId', 'name email'),
                Project_1.default.find(query).limit(500).populate('studentId', 'name').populate('mentorId', 'name'),
                Internship_1.default.find(query).limit(500).populate('studentId', 'name'),
                Certification_1.default.find(query).limit(500).populate('studentId', 'name')
            ]);
            data = [
...
// Generate PDF report
const generatePDF = async (data, category, filters) => {
    return new Promise((resolve, reject) => {
        try {
            ...
            doc.moveDown();
            doc.fontSize(12).text(`Generated on: ${(0, moment_1.default)().format('DD-MM-YYYY HH:mm:ss')}`, { align: 'center' });
            doc.moveDown();
            // Filters summary
            ...
            if (category === 'comprehensive') {
                ...
                data.forEach((section) => {
                    ...
                    if (section.data && section.data.length > 0) {
                        section.data.slice(0, 10).forEach((item, index) => {
                            let name = 'N/A';
                ...
                data.forEach((item, index) => {
                    ...
                    doc.fontSize(12).text(`${index + 1}. ${name}`);
                    if (item.department)
                        doc.fontSize(10).text(`   Department: ${item.department}`);
                    if (item.year)
                        doc.fontSize(10).text(`   Year: ${item.year}`);
                    if (item.status)
                        doc.fontSize(10).text(`   Status: ${item.status}`);
                    if (item.totalPoints)
                        doc.fontSize(10).text(`   Points: ${item.totalPoints}`);
                    doc.moveDown();
                });
            }
            doc.end();
        ...
        for (const section of data) {
            ...
            sectionData.forEach((item, idx) => {
                ...
                if (section.section === 'Students') {
                    ...
                    rowData.createdAt = item.createdAt ? (0, moment_1.default)(item.createdAt).format('DD-MM-YYYY') : 'N/A';
                }
                else if (section.section === 'Mentors') {
                    rowData.name = item.userId?.name || item.name || 'N/A';
                    rowData.email = item.userId?.email || item.email || 'N/A';
                    ...
                    rowData.status = item.status || 'N/A';
                    rowData.duration = item.from && item.to ? `${(0, moment_1.default)(item.from).format('DD-MM-YYYY')} to ${(0, moment_1.default)(item.to).format('DD-MM-YYYY')}` : 'N/A';
                    rowData.createdAt = item.createdAt ? (0, moment_1.default)(item.createdAt).format('DD-MM-YYYY') : 'N/A';
                }
                else if (section.section === 'Certifications') {
                    rowData.studentName = item.studentId?.name || 'N/A';
                    ...
                    rowData.status = item.status || 'N/A';
                    rowData.issueDate = item.from ? (0, moment_1.default)(item.from).format('DD-MM-YYYY') : 'N/A';
                    rowData.createdAt = item.createdAt ? (0, moment_1.default)(item.createdAt).format('DD-MM-YYYY') : 'N/A';
                }
                else {
...
// Generate CSV report
const generateCSV = async (data, category, filters) => {
    if (category === 'comprehensive') {
            ...
            if (section.data && section.data.length > 0) {
                ...
                switch (section.section) {
                    case 'Students':
                        ...
                        rows = section.data.map((item, idx) => [
                            idx + 1,
    ...
    switch (category) {
        case 'students':
            ...
            records = data.map((item, index) => ({
                ...
                createdAt: item.createdAt ? (0, moment_1.default)(item.createdAt).format('DD-MM-YYYY') : 'N/A'
            }));
            break;
        case 'mentors':
            header = [
        ...
        case 'internships':
            ...
            records = data.map((item, index) => ({
                ...
                type: item.type || 'N/A',
                status: item.status || 'N/A',
                duration: item.from && item.to ? `${(0, moment_1.default)(item.from).format('DD-MM-YYYY')} to ${(0, moment_1.default)(item.to).format('DD-MM-YYYY')}` : 'N/A',
                createdAt: item.createdAt ? (0, moment_1.default)(item.createdAt).format('DD-MM-YYYY') : 'N/A'
            }));
            break;
        case 'certifications':
            ...
            records = data.map((item, index) => ({
                ...
                status: item.status || 'N/A',
                issueDate: item.from ? (0, moment_1.default)(item.from).format('DD-MM-YYYY') : 'N/A',
                createdAt: item.createdAt ? (0, moment_1.default)(item.createdAt).format('DD-MM-YYYY') : 'N/A'
            }));
            break;
        case 'performance':
    ...
    const csvContent = fs_1.default.readFileSync('temp.csv', 'utf8');
    fs_1.default.unlinkSync('temp.csv');
    return csvContent;
};
    ...
    try {
        ...
        switch (type) {
            case 'pdf':
                ...
                fileName = `${category}_report_${(0, moment_1.default)().format('DDMMYYYY_HHmmss')}.pdf`;
                mimeType = 'application/pdf';
                break;
            case 'excel':
                fileBuffer = await generateExcel(data, category, filter);
                fileName = `${category}_report_${(0, moment_1.default)().format('DDMMYYYY_HHmmss')}.xlsx`;
                mimeType = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet';
                break;
            case 'csv':
                const csvContent = await generateCSV(data, category, filter);
                fileBuffer = Buffer.from(csvContent);
                fileName = `${category}_report_${(0, moment_1.default)().format('DDMMYYYY_HHmmss')}.csv`;
                mimeType = 'text/csv';
                break;
            default:
...
const previewReport = async (req, res) => {
    try {
        ...
        if (!previewFilter.startDate && !previewFilter.endDate) {
            ...
            previewFilter.startDate = (0, moment_1.default)(start).format('YYYY-MM-DD');
            previewFilter.endDate = (0, moment_1.default)(end).format('YYYY-MM-DD');
        }
...
const getReportHistory = async (req, res) => {
    try {
        ...
        const reports = await Report_1.default.find({ generatedBy: userId })
            .sort({ generatedAt: -1 })
            .skip(skip)
            .limit(limit)
            .populate('generatedBy', 'name email');
        const total = await Report_1.default.countDocuments({ generatedBy: userId });
        res.json({
            success: true,
            data: reports,
            pagination: {
...
const downloadReport = async (req, res) => {
    try {
        ...
        const userId = req.user?.id;
        const report = await Report_1.default.findOne({ _id: reportId, generatedBy: userId });
        if (!report) {
        ...
        const filePath = path_1.default.join(__dirname, '../../uploads/reports', path_1.default.basename(report.fileUrl));
        ...
        return res.download(filePath, path_1.default.basename(report.fileUrl), (err) => {
            if (err) {
...
const deleteReport = async (req, res) => {
    try {
        const { reportId } = req.params;
        const userId = req.user?.id;
        const report = await Report_1.default.findOneAndDelete({ _id: reportId, generatedBy: userId });
        if (!report) {
            return res.status(404).json({ success: false, message: 'Report not found' });
        }
...
const getReportStats = async (req, res) => {
    try {
        ...
        const totalReports = await Report_1.default.countDocuments({ generatedBy: userId });
        const completedReports = await Report_1.default.countDocuments({ generatedBy: userId, status: 'completed' });
        const failedReports = await Report_1.default.countDocuments({ generatedBy: userId, status: 'failed' });
        // Get reports by type
        const reportsByType = await Report_1.default.aggregate([
            { $match: { generatedBy: userId } },
            { $group: { _id: '$reportType', count: { $sum: 1 } } }
        ]);
        // Get reports by category
        const reportsByCategory = await Report_1.default.aggregate([
            { $match: { generatedBy: userId } },
            { $group: { _id: '$category', count: { $sum: 1 } } }
        ]);
        // Get recent reports
        const recentReports = await Report_1.default.find({ generatedBy: userId })
            .sort({ generatedAt: -1 })
            .limit(5)
...
const getMentorsReport = async (req, res) => {
    try {
